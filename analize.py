...
for i in range(25000):
        ...
        if 0 < sc < 7: defect.append(random.choice([0,1]))
        elif sc == 7 : defect.append(1)
        else: defect.append(0)
...
target = data['Defect']
indicators = data.drop(['Defect', 'Extruder signal zone 1', 'Extruder signal zone 2', 'Extruder signal zone 3', 'Extruder signal pressure', 'Chiller signal', 'Streighthening signal', 'Rolling signal'], axis=1)

X_train, x_test, Y_train, y_test = train_test_split(indicators, target, test_size=0.01)
...
print('Вероятность возникновения дефекта:', np.mean(model.predict(x_test)) * 100, '%')
...
def prediction(path, model):
  data = pd.read_csv(path , sep=',')
  area = data.drop(['Defect', 'Extruder signal zone 1', 'Extruder signal zone 2', 'Extruder signal zone 3', 'Extruder signal pressure', 'Chiller signal', 'Streighthening signal', 'Rolling signal'], axis=1)
  area = area.iloc[len(area) - 1000 :]
  print('Вероятность возникновения дефекта:', np.mean(model.predict(area)) * 100, '%')
